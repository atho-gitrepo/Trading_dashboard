...
export function useStats(activeSignals: TradeSignal[], resolvedSignals: TradeSignal[]) {
  const stats = useMemo<DashboardStats>(() => {
    ...
    const profitable = allResolved.filter(s => 
      s.status === 'PROFIT' || s.status === 'PARTIAL_PROFIT'
    );
    const losing = allResolved.filter(s => 
      s.status === 'LOSS' || s.status === 'PARTIAL_LOSS'
    );
    ...
    const winRate = allResolved.length > 0 
      ? (profitable.length / allResolved.length) * 100 
      : 0;
  ...
  const dailyStats = useMemo<DailyStats[]>(() => {
    ...
    resolvedSignals.forEach(s => {
      const date = s.exit_time 
        ? new Date(s.exit_time).toDateString()
        : s.resolved_at 
          ? new Date(s.resolved_at).toDateString()
