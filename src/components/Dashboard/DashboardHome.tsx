...
export const DashboardHome: React.FC = () => {
  ...
  const weeklyData = React.useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayStat = dailyStats.find(d => new Date(d.date).toDateString() === dateStr);
      data.push({
    ...
    <div className="min-h-screen bg-gray-950 text-white p-4 pb-20">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <button className="bg-gray-800 px-4 py-2 rounded-lg text-sm hover:bg-gray-700 transition-colors">
            Pro
          </button>
          <button className="bg-blue-600 px-4 py-2 rounded-lg text-sm hover:bg-blue-700 transition-colors">
            Upgrade
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <button className="bg-gray-800 p-3 rounded-lg text-center text-sm hover:bg-gray-700 transition-colors">
          Mindset
        </button>
        <button className="bg-gray-800 p-3 rounded-lg text-center text-sm hover:bg-gray-700 transition-colors">
          Pre-Session
      ...
      <div className="bg-gray-800 rounded-xl p-4 mb-6">
        ...
        <p className="text-xs text-gray-400 mt-2">
          Active: {stats.totalActive} | Avg Score: {stats.avgScore.toFixed(1)}
        </p>
      </div>
      ...
      <div className="bg-gray-800 rounded-xl p-4 text-center mt-4">
        <p className="text-sm text-gray-400">
          {stats.totalActive > 0 
            ? `${stats.totalActive} active signal${stats.totalActive > 1 ? 's' : ''}` 
            : 'No active signals'}
        </p>
        {stats.totalActive > 0 && (
          <p className="text-xs text-blue-400 mt-1">
            Check Trades tab for details →
          </p>
        )}
      </div>
