...
export function useSignals(): UseSignalsResult {
  ...
  const refresh = useCallback(() => {
    ...
    const unsubActive = firebaseQueries.subscribeActiveSignals(
      (signals) => {
    ...
    unsubscribesRef.current.push(unsubActive);

    const unsubResolved = firebaseQueries.subscribeResolvedSignals(
      (signals) => {
    ...
    unsubscribesRef.current.push(unsubResolved);
  }, []);
